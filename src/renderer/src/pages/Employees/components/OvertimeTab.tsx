    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-slate-900 dark:text-white flex items-center gap-2">
          <AlarmClock size={16} /> Overtime Records
        </h3>
      ...
      {overtimeRecords.length > 0 && (
        <div className="flex gap-4 flex-wrap">
          <div className="px-4 py-2 rounded-lg bg-slate-50 dark:bg-slate-700">
            ...
            <div className="text-xs text-slate-500">Total OT Hours</div>
          </div>
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 dark:bg-slate-700/50">
              <tr>
                {['Date', 'Hours', 'Multiplier', 'Reason', 'Status', 'Approved By', ''].map(h => (
              ...
              {overtimeRecords.map(o => (
                <tr key={o.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                  ...
                  <td className="px-4 py-3">
                    {o.approved ? (
                      <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">Approved</span>
                    ) : (
                      <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">Pending</span>
                    )}
                  </td>

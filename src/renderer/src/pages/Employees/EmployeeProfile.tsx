...
export default function EmployeeProfilePage() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      {/* ── Attendance Modal ─────────────────────────────────────────────── */}
      <Modal isOpen={s.showAttModal} onClose={() => s.setShowAttModal(false)} title="Log Attendance">
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3">
            <button onClick={() => s.setShowAttModal(false)} className="px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 text-sm text-slate-700 dark:text-slate-300">Cancel</button>
            <button onClick={s.saveAttendance} disabled={s.savingAtt} className="btn-primary">{s.savingAtt ? 'Saving…' : 'Save'}</button>
          </div>
        </div>
      </Modal>

      {/* ── Payroll Modal ────────────────────────────────────────────────── */}
      <Modal isOpen={s.showPayModal} onClose={() => s.setShowPayModal(false)} title="Add / Edit Payroll">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Paid Date</label>
          ...
          <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-700/50 flex items-center justify-between">
            <span className="text-sm text-slate-600 dark:text-slate-400">Net Pay</span>
            <span className="text-xl font-bold text-slate-900 dark:text-white">${s.netPay.toFixed(2)}</span>
          </div>
          <div className="flex justify-end gap-3">
            <button onClick={() => s.setShowPayModal(false)} className="px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 text-sm text-slate-700 dark:text-slate-300">Cancel</button>
            <button onClick={s.savePayroll} disabled={s.savingPay} className="btn-primary">{s.savingPay ? 'Saving…' : 'Save Payroll'}</button>
          </div>
        </div>
      </Modal>

      {/* ── Note Modal ───────────────────────────────────────────────────── */}
      <Modal isOpen={s.showNoteModal} onClose={() => s.setShowNoteModal(false)} title="Add Note">
        <div className="space-y-4">
          <textarea
            value={s.noteText} onChange={e => s.setNoteText(e.target.value)}
            rows={4} placeholder="Enter note…"
            className="w-full px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm resize-none"
          />
          <div className="flex justify-end gap-3">
            <button onClick={() => s.setShowNoteModal(false)} className="px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 text-sm text-slate-700 dark:text-slate-300">Cancel</button>
            <button onClick={s.saveNote} disabled={s.savingNote || !s.noteText.trim()} className="btn-primary">{s.savingNote ? 'Saving…' : 'Add Note'}</button>
          </div>
        </div>
      </Modal>

      {/* ── Shift Modal ──────────────────────────────────────────────────── */}
      <Modal isOpen={s.showShiftModal} onClose={() => s.setShowShiftModal(false)} title="Add Shift">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Date</label>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Shift Type</label>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Break (minutes)</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Notes</label>
            <input type="text" value={s.shiftForm.notes} onChange={e => s.setShiftForm(p => ({ ...p, notes: e.target.value }))} placeholder="Optional notes"
              className="w-full px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm" />
          </div>
          <div className="flex justify-end gap-3">
            <button onClick={() => s.setShowShiftModal(false)} className="px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 text-sm text-slate-700 dark:text-slate-300">Cancel</button>
            <button onClick={s.saveShift} disabled={s.savingShift} className="btn-primary">{s.savingShift ? 'Saving…' : 'Add Shift'}</button>
          </div>
        </div>
      </Modal>

      {/* ── Overtime Modal ───────────────────────────────────────────────── */}
      <Modal isOpen={s.showOTModal} onClose={() => s.setShowOTModal(false)} title="Log Overtime">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Date</label>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Rate Multiplier</label>
              <select value={s.otForm.multiplier} onChange={e => s.setOtForm(p => ({ ...p, multiplier: Number(e.target.value) }))}
                className="w-full px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm">
                <option value={1.0}>1× (Regular)</option>
          ...
          <div>
            ...
            <input type="text" value={s.otForm.reason} onChange={e => s.setOtForm(p => ({ ...p, reason: e.target.value }))} placeholder="Optional reason"
              className="w-full px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm" />
          </div>
          {/* Pay estimate */}
          {s.emp.salary > 0 && (() => {
            ...
            return (
              <div className="p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 flex items-center justify-between">
                <span className="text-sm text-amber-700 dark:text-amber-400">Estimated OT Pay</span>
          ...
          <div className="flex justify-end gap-3">
            <button onClick={() => s.setShowOTModal(false)} className="px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 text-sm text-slate-700 dark:text-slate-300">Cancel</button>
            <button onClick={s.saveOvertime} disabled={s.savingOT} className="btn-primary">{s.savingOT ? 'Saving…' : 'Log Overtime'}</button>
          </div>
        </div>
      </Modal>
      {/* ── Confirm Dialog ───────────────────────────────────────────────── */}
      <Modal isOpen={!!s.confirm} onClose={() => s.setConfirm(null)} title="Confirm Action" size="sm">
        <div className="space-y-4">
          <p className="text-sm text-slate-600 dark:text-slate-400">{s.confirm?.message}</p>
          <div className="flex justify-end gap-3">
            <button onClick={() => s.setConfirm(null)} className="px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 text-sm text-slate-700 dark:text-slate-300">Cancel</button>
            <button onClick={s.confirm?.onConfirm} className="px-4 py-2 rounded-lg bg-red-600 text-white text-sm font-medium hover:bg-red-700 transition-colors">Delete</button>
          </div>
        </div>
      </Modal>
    </div>

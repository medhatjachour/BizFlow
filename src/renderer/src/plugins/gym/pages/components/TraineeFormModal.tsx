      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white dark:bg-slate-800 flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700 rounded-t-2xl">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-semibold text-slate-900 dark:text-white">
              {initial ? t('gymEditTrainee') : t('gymNewTrainee')}
            </h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input className={inputCls} value={form.name} onChange={set('name')} placeholder={t('gymMemberFullName')} required />
          </div>
          {/* Phone + Email */}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={labelCls}>{t('gymAge')}</label>
              <input className={inputCls} type="number" min="5" max="100" value={form.age} onChange={set('age')} placeholder="e.g. 25" />
            </div>
            <div>
              <label className={labelCls}>{t('gymGender')}</label>
              <select className={inputCls} value={form.gender} onChange={set('gender')}>
                <option value="">{t('gymGenderSelect')}</option>
                <option value="male">{t('gymGenderMale')}</option>
                <option value="female">{t('gymGenderFemale')}</option>
                <option value="other">{t('gymGenderOther')}</option>
              </select>
            </div>
          </div>
          {/* National ID */}
          ...
          <div>
            ...
            <textarea className={inputCls} rows={2} value={form.notes} onChange={set('notes')} placeholder={t('gymHealthNotes')} />
          </div>
          {/* Actions */}
          <div className="flex gap-3 pt-1">
            ...
            <button type="submit" disabled={saving} className="flex-1 px-4 py-2 rounded-xl bg-orange-500 hover:bg-orange-600 text-white text-sm font-medium transition-colors disabled:opacity-60">
              {saving ? <Loader2 size={14} className="animate-spin mx-auto" /> : initial ? t('gymSave') : t('gymAddTrainee')}
            </button>
          </div>
        </form>
      </div>

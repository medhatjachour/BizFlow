...
export default function TablesTab() {
  ...
  const load = async () => {
    ...
    catch { setError(t('restaurantLoadTablesFailed')) }
  ...
  const handleDelete = async (id: string) => {
    if (!confirm(t('restaurantRemoveTableConfirm'))) return
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-sm font-medium transition-colors">
            <Plus className="w-4 h-4" /> {t('restaurantAddTable')}
          </button>
        </div>
      </div>
            ...
            return (
              <div key={table.id} className={`relative rounded-xl border-2 p-4 transition-all ${style.bg}`}>
                ...
                <div className="flex items-center gap-1 text-slate-500 dark:text-slate-400 text-sm mb-2">
                  <Users className="w-3.5 h-3.5" /> {table.capacity} {t('restaurantSeats')}
                </div>
                ...
                >
                  <option value="available">{t('restaurantAvailable')}</option>
                  <option value="occupied">{t('restaurantOccupied')}</option>
                  <option value="reserved">{t('restaurantReserved')}</option>
                  <option value="cleaning">{t('restaurantCleaning')}</option>
                </select>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
            <h3 className="text-lg font-semibold text-slate-900 dark:text-white">{editing ? t('restaurantEditTable') : t('restaurantAddTable')}</h3>
            <label className="block">
              <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('restaurantTableNumber')} *</span>
            ...
            <label className="block">
              <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('restaurantCapacity')} *</span>
              <input type="number" required min="1" value={form.capacity} onChange={e => setForm(f => ({ ...f, capacity: e.target.value }))}
                className="mt-1 w-full rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" />
            </label>
            <label className="block">
              <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('restaurantSection')}</span>
              <input type="text" placeholder={t('restaurantSectionPlaceholder')} value={form.section} onChange={e => setForm(f => ({ ...f, section: e.target.value }))}
                className="mt-1 w-full rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" />
            </label>
            {editing && (
              <label className="block">
                <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('restaurantStatus')}</span>

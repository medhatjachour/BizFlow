...
function BatchManagementModal({ material, onClose }: { material: Material; onClose: () => void }) {
  ...
  function startEdit(b: Batch) {
    setEditingId(b.id)
    setEditBatchNo(b.batchNumber ?? '')
    setEditQty(b.quantity.toString())
    setEditExpiry(b.expiryDate ? b.expiryDate.slice(0, 10) : '')
    setEditCost(b.costPerUnit?.toString() ?? '')
  ...
  function resetNew() {
    setNewBatchNo(''); setNewQty('0'); setNewExpiry('')
    setNewCost(''); setNewSupplier(''); setNewNotes('')
  }
  ...
  async function handleAdd() {
    ...
    try {
      await window.api.clinic.materialBatches.create(material.id, {
        batchNumber: newBatchNo.trim() || undefined,
        quantity: parseFloat(newQty) || 0,
        expiryDate: newExpiry || undefined,
        costPerUnit: newCost ? parseFloat(newCost) : undefined,
        supplier: newSupplier.trim() || undefined,
        notes: newNotes.trim() || undefined,
      })
      resetNew(); setAdding(false)
      showToast('success', t('createdSuccessfully'))
      load()
    } catch { showToast('error', t('errorSavingRecord')) }
    finally { setSaving(false) }
  ...
  async function handleUpdate(id: string) {
    ...
    try {
      await window.api.clinic.materialBatches.update(id, {
        batchNumber: editBatchNo.trim() || undefined,
        quantity: parseFloat(editQty) || 0,
        expiryDate: editExpiry || undefined,
        costPerUnit: editCost ? parseFloat(editCost) : undefined,
  ...
  function BatchForm({ mode }: { mode: 'add' | 'edit' }) {
    const isAdd = mode === 'add'
    const batchNo    = isAdd ? newBatchNo    : editBatchNo
    const qty        = isAdd ? newQty        : editQty
    const expiry     = isAdd ? newExpiry     : editExpiry
    const cost       = isAdd ? newCost       : editCost
    const supplier   = isAdd ? newSupplier   : editSupplier
    const notes      = isAdd ? newNotes      : editNotes
    return (
      <div className="space-y-3 p-4 rounded-xl border-2 border-teal-200 dark:border-teal-800 bg-teal-50/50 dark:bg-teal-900/10">
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input className={inputCls} value={batchNo} onChange={e => isAdd ? setNewBatchNo(e.target.value) : setEditBatchNo(e.target.value)} placeholder={t('optional')} />
          </div>
          <div>
            <label className={labelCls}>{t('materialQuantity')} *</label>
            <input type="number" min="0" step="0.01" className={inputCls} value={qty} onChange={e => isAdd ? setNewQty(e.target.value) : setEditQty(e.target.value)} />
          </div>
          <div>
            <label className={labelCls}>{t('materialExpiryDate')}</label>
            <input type="date" className={inputCls} value={expiry} onChange={e => isAdd ? setNewExpiry(e.target.value) : setEditExpiry(e.target.value)} />
          </div>
          <div>
            <label className={labelCls}>{t('materialCostPerUnit')}</label>
            <input type="number" min="0" step="0.01" className={inputCls} value={cost} onChange={e => isAdd ? setNewCost(e.target.value) : setEditCost(e.target.value)} placeholder={t('optional')} />
          </div>
          <div>
            <label className={labelCls}>{t('supplier')}</label>
            <input className={inputCls} value={supplier} onChange={e => isAdd ? setNewSupplier(e.target.value) : setEditSupplier(e.target.value)} placeholder={t('optional')} />
          </div>
          <div>
            <label className={labelCls}>{t('notes')}</label>
            <input className={inputCls} value={notes} onChange={e => isAdd ? setNewNotes(e.target.value) : setEditNotes(e.target.value)} placeholder={t('optional')} />
          </div>
        </div>
        {!isAdd && (
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" checked={editActive} onChange={e => setEditActive(e.target.checked)} className="h-4 w-4 rounded accent-teal-600" />
            <span className="text-sm text-slate-700 dark:text-slate-300">{t('materialActive')}</span>
          </label>
        )}
        <div className="flex gap-2">
          <button onClick={isAdd ? handleAdd : () => handleUpdate(editingId!)} disabled={saving} className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold disabled:opacity-50">
            {saving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Save className="h-3.5 w-3.5" />} {t('save')}
          </button>
          <button onClick={() => isAdd ? (setAdding(false), resetNew()) : setEditingId(null)} className="px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800">
            {t('cancel')}
          </button>
        </div>
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/60 backdrop-blur-sm overflow-y-auto py-8 px-4" onClick={onClose}>
      <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-2xl shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-6 py-4 space-y-3 max-h-[60vh] overflow-y-auto">
            ...
            batches.map(b => {
              ...
              return (
                <div key={b.id} className={`rounded-xl border overflow-hidden ${!b.isActive ? 'opacity-50 border-slate-200 dark:border-slate-700' : 'border-slate-200 dark:border-slate-700'}`}>
                  {editingId === b.id ? (
                    <div className="p-3"><BatchForm mode="edit" /></div>
                  ) : (
                    <div className="flex items-start gap-3 px-4 py-3 bg-white dark:bg-slate-800">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <button onClick={() => startEdit(b)} className="p-1.5 rounded-lg text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20"><Pencil className="h-3.5 w-3.5" /></button>
                        <button onClick={() => handleDelete(b.id)} className="p-1.5 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20"><Trash2 className="h-3.5 w-3.5" /></button>
                      </div>
                    </div>
                  )}
                </div>
          ...
          {adding && <BatchForm mode="add" />}
        </div>

        <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-700">
          <button
            onClick={() => { setAdding(true); resetNew() }}
            disabled={adding}
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold transition-colors disabled:opacity-50"
          >
...
export default function MaterialsTab() {
  ...
  const [logLossMaterial, setLogLossMaterial] = useState<Material | null>(null)
  const [logExpiryMaterial, setLogExpiryMaterial] = useState<Material | null>(null)
    ...
    <div className="space-y-5">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        <button
                          ...
                          title={t('clinicMaterialAdjustStock')}
                        ...
                        <button
                          onClick={() => setLogLossMaterial(m)}
                          title={t('logMaterialLoss')}
                        ...
                        <button
                          onClick={() => setLogExpiryMaterial(m)}
                          title={t('logMaterialExpiry')}
      ...
      {adjustingMaterial && (
        <AdjustStockModal
          ...
          onSaved={() => { setAdjustingMaterial(null); load() }}
        />
      )}
      {logLossMaterial && (
        <LogMaterialLossModal
          material={logLossMaterial}
          onClose={() => setLogLossMaterial(null)}
          onSaved={() => { setLogLossMaterial(null); load() }}
        />
      )}
      {logExpiryMaterial && (
        <LogMaterialExpiryModal
          material={logExpiryMaterial}
          onClose={() => setLogExpiryMaterial(null)}
          onSaved={() => { setLogExpiryMaterial(null); load() }}
        />
      )}
      {managingBatchesFor && (

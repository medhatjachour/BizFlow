...
export const DeleteExpenseModal: React.FC<Props> = ({ expense, onClose, onConfirm }) => {
  const { t } = useLanguage()
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-in fade-in-50 duration-150">
      ...
      >
        ...
        <h3 className="text-base font-extrabold text-slate-900 dark:text-white">
          {t('confirmDelete') || 'Delete Expense Record'}?
        </h3>
        <p className="mt-2 text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
          Are you sure you want to delete <strong className="text-slate-800 dark:text-slate-200">"{expense.description}"</strong> (${expense.amount.toFixed(2)})? This action cannot be undone.
        </p>

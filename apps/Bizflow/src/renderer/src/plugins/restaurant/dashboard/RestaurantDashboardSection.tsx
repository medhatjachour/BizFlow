...
export default function RestaurantDashboardSection({ refreshSignal }: Props) {
  ...
  const load = async () => {
    try {
      ...
      const revenueToday =
        revenueR.status === 'fulfilled' ? (revenueR.value?.total ?? revenueR.value ?? 0) : 0

      const data: RestaurantData = {
        tables,
        reservations,
        activeOrders,
        allOrdersToday: allOrders,
        revenueToday
      }
      setRaw(data)
      ...
      <div className="space-y-4 animate-pulse">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {Array.from({ length: 5 }).map((_, i) => (
            <div
              key={i}
              className="bg-white dark:bg-slate-800 rounded-xl h-24 border border-slate-200 dark:border-slate-700"
            />
          ))}
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          {Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="bg-white dark:bg-slate-800 rounded-xl h-48 border border-slate-200 dark:border-slate-700"
            />
          ))}
        </div>
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 px-1">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">{t('restaurantOverview')}</p>
        </div>
      </div>
      ...
      <KpiSection sectionKey="restaurant:dashboard-RestaurantDashboardSection">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <StatCard
            ...
            value={`${occupiedCount}/${raw.tables.length}`}
          ...
          <StatCard
            icon={DollarSign}
            label="Revenue Today"
            value={`$${Number(raw.revenueToday).toFixed(0)}`}
          ...
          <StatCard
            icon={Clock}
            label="Avg Turnover"
            value={fmtMin(metrics?.avgTurnoverMin ?? 0)}
            ...
            <div className="mt-2 grid grid-cols-2 sm:grid-cols-3 gap-1 text-xs text-center text-slate-500">
              <div>
                <span className="block font-semibold text-slate-800 dark:text-white">
                  {heatmap.periods.morning}
                </span>
                Morning
              </div>
              <div>
                <span className="block font-semibold text-slate-800 dark:text-white">
                  {heatmap.periods.afternoon}
                </span>
                Afternoon
              </div>
              <div>
                <span className="block font-semibold text-slate-800 dark:text-white">
                  {heatmap.periods.evening}
                </span>
                Evening
              </div>
            </div>
            ...
            <div className="space-y-2 max-h-52 overflow-y-auto pr-1">
              {raw.activeOrders.map((order: any, i: number) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-7 h-7 rounded flex items-center justify-center text-xs font-bold text-white flex-shrink-0 ${TABLE_COLORS[order.status] || 'bg-rose-500'}`}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {elapsed !== null && (
                        <p
                          className={`text-xs ${elapsed > 30 ? 'text-red-500' : 'text-slate-500'}`}
                        >
                          {elapsed}m ago
                        </p>
                      )}
            ...
            <div className="space-y-2 max-h-44 overflow-y-auto pr-1">
              {raw.reservations.slice(0, 6).map((res: any, i: number) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-slate-800 dark:text-white truncate">
                      {res.guestName || res.name || 'Guest'}
                    </p>
                    <p className="text-xs text-slate-500 flex items-center gap-1">
                      <Users size={10} />
                      {res.partySize || res.guestCount || 1} guests
                    </p>
                  </div>
                  <span
        ...
        <div className="space-y-2">
          {[
            ...
            >
              <Icon
                size={15}
                className="text-rose-600 flex-shrink-0 group-hover:scale-110 transition-transform"
              />
              <div>

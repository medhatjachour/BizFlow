...
import { Users, Clock, ArrowRightLeft, GitMerge, UserCheck, Edit2, Trash2 } from 'lucide-react'
...
export const TableCard: React.FC<Props> = ({
  ...
  return (
    <div
      role="group"
      tabIndex={0}
      aria-label={t('restFloorTileAria', { number: table.number, status: statusLabel })}
      ...
      className={`group relative rounded-2xl border-2 p-4 cursor-pointer transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 ${cfg.bg} ${cfg.border} ${cfg.glow} shadow-sm hover:shadow-md flex flex-col justify-between`}
    >
      ...
      <div>
        ...
        <div className="flex items-center justify-between gap-2 text-xs text-slate-500 dark:text-slate-400 mt-2 mb-3">
          <span className="truncate font-medium">{table.section}</span>
      ...
      <div className="my-2 bg-white/80 dark:bg-slate-900/60 rounded-xl p-2.5 border border-slate-200/60 dark:border-slate-700/60 space-y-1.5 min-h-[54px] flex flex-col justify-center">
        {openOrder ? (
          <div className="space-y-1">
            ...
            <div className="text-[11px] text-slate-500 truncate flex items-center justify-between gap-2">
              <span className="truncate">{openOrder.serverName || t('restTableLeadStaff')}</span>
              <span className="text-slate-400 whitespace-nowrap">
                {t('restTableItemsCount', { count: openOrder.items.length })}
              </span>
            </div>
          </div>
        ) : nextReservation ? (
          <div className="text-xs text-blue-600 dark:text-blue-400 font-medium truncate">
            {t('restTableReservationShort', {
              time: new Date(nextReservation.date).toLocaleTimeString([], {
                hour: '2-digit',
                minute: '2-digit'
              }),
              name: nextReservation.customerName,
              count: nextReservation.partySize
            })}
          </div>
        ) : (
      ...
      <div className="pt-2 border-t border-slate-200/50 dark:border-slate-700/50 flex items-center justify-between gap-1">
        {table.status === 'available' ? (
          ...
          >
            <UserCheck className="w-3.5 h-3.5" /> {t('restTableQuickSeat')}
          </button>
          ...
          <div className="flex items-center gap-1 w-full">
            <button
              ...
              title={t('restTableTransfer')}
              aria-label={t('restTableTransfer')}
            ...
            <button
              ...
              title={t('restTableMerge')}
              aria-label={t('restTableMerge')}
              className="p-1.5 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:text-amber-600 transition-colors"
            >
            ...
            >
              {table.status === 'occupied' ? t('restTableBillOut') : t('restTableMarkCleanShort')}
            </button>
          </div>
        ) : (
          <button
        ...
        <div className="flex items-center gap-0.5">
          <button
            ...
            title={t('restTableEdit')}
            aria-label={t('restTableEdit')}
          ...
          <button
            ...
            title={t('restTableDelete')}
            aria-label={t('restTableDelete')}

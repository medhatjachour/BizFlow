  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-slate-400">
        ...
        <p className="text-xs">{t('phSaLoading')}</p>
      </div>
    )
  }
  ...
  if (sales.length === 0) {
    return (
      <div className="text-center py-20 text-slate-400">
        <p className="text-sm font-medium">{t('phNoSales')}</p>
        <p className="text-xs mt-0.5">{t('phSaEmptyHint')}</p>
      </div>
    )
  }
      ...
      <table className="w-full text-xs text-left">
        <thead>
          <tr className="text-slate-400 border-b border-slate-100 dark:border-slate-800 font-semibold bg-slate-50/40 dark:bg-slate-900/30">
            <th className="px-4 py-3">{t('phSaSaleNumber')}</th>
            <th className="px-4 py-3">{t('restaurantDateAndTime')}</th>
            <th className="px-4 py-3">{t('salesUiCustomer')}</th>
            <th className="px-4 py-3 text-center">{t('inventoryUiItems')}</th>
            <th className="px-4 py-3 text-right">{t('salesUiTotal')}</th>
            <th className="px-4 py-3 text-center">{t('salesUiPayment')}</th>
            <th className="px-4 py-3 text-center">{t('status')}</th>
            <th className="px-4 py-3 text-right">{t('phSaAction')}</th>
          </tr>
        </thead>
            ...
            return (
              ...
              >
                ...
                <td className="px-4 py-2.5 font-medium">
                  {s.customerName || <span className="text-slate-400 italic">{t('vetJourneyWalkinTitle')}</span>}
                </td>
                ...
                <td className="px-4 py-2.5 text-right font-bold text-slate-900 dark:text-slate-100">
                  ...
                  {outstanding > 0.005 && (
                    <span className="block text-[10px] text-red-500 font-semibold">
                      -{t('phAmountDue', { amount: money(outstanding) })}
                    </span>
                  )}
                </td>

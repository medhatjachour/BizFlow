import { X, Edit2, Phone, MapPin, StickyNote, Star, TrendingUp, Coffee, ShoppingBag, CalendarClock } from 'lucide-react'
...
export function CustomerDrawer({ profile, loading, onClose, onEdit }: Props) {
  ...
  const avgOrder = profile && profile.orders.length > 0 
    ? profile.totalSpent / profile.orders.length 
    : 0

  const stats = {
    totalSpent: formatCurrency(profile?.totalSpent),
    visitCount: String(profile?.visitCount ?? 0),
    orders: String(profile?.orders?.length ?? 0),
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-4 min-w-0">
              {loading ? (
                ...
                <div className={`relative w-14 h-14 rounded-full bg-gradient-to-br ${getAvatarGradient(profile?.name ?? '')} flex items-center justify-center text-white font-bold text-lg shrink-0`}>
                ...
                {loading ? (
                  ...
                  <>
                    <div className="flex items-center gap-2 flex-wrap">
                      <h2 className="text-lg font-bold text-slate-900 dark:text-white truncate">{profile?.name}</h2>
                      {profile?.isVip && <span className="px-2 py-0.5 text-[10px] font-bold bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 rounded-full">VIP</span>}
                    </div>
              ...
              {PROFILE_STATS.map(({ key, label, icon: Icon, color }) => {
                ...
                return (
                  <div key={key} className={`p-4 rounded-xl ${styles.bg} border ${styles.border}`}>
                    <div className={`w-8 h-8 rounded-lg ${styles.bg} ${styles.text} flex items-center justify-center mb-2`}>
              ...
              {profile.orders.length === 0 ? (
                ...
                <div className="space-y-2">
                  {profile.orders.map(order => (
                    <div key={order.id} className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700/50">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="font-medium text-slate-900 dark:text-white text-sm">#{order.orderNumber}</span>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {order.items.slice(0, 4).map((item, i) => (
                          <span key={i} className="px-2 py-0.5 text-[11px] bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 rounded-md border border-slate-200 dark:border-slate-700">

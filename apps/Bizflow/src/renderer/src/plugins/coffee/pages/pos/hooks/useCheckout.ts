...
export function useCheckout() {
  ...
  const reprintLast = useCallback(async (toast: any) => {
  ...
  // ── Full checkout ──
  const checkout = useCallback(async (params: CheckoutParams, pm: PaymentMethod) => {
    setChecking(true)
    try {
      ...
      const order = await window.api.coffee.orders.create({
        ...
        deliveryAddress: ck.orderType === 'delivery' ? (ck.customerAddress || undefined) : undefined,
        customerId,
        cashierId,
        shiftId,
      ...
      const tableLabel = ck.orderType === 'dine_in' && ck.selectedTable
        ? (() => {
  ...
  // ── Quick checkout ──
  const quickCheckout = useCallback(async (params: {
    cart: CartItem[]
    cashierId?: string
    shiftId?: string
    toast: any

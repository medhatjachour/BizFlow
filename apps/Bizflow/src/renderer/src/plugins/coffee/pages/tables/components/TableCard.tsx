            ...
            <div className="space-y-1">
              {order.items.slice(0, 3).map((item) => (
                <div key={item.id} className="flex items-center justify-between text-xs">
                  ...
                  <span
                    className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium uppercase tracking-wide ${ITEM_ST[item.status] ?? ITEM_ST.pending}`}
                  >
                    {item.status}
                  </span>
                </div>
              ...
              {order.items.length > 3 && (
                <div className="text-[10px] text-slate-400">+ {order.items.length - 3} more</div>
              )}
            </div>
          </div>
      ...
      <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-700/50">
        <div className="flex items-center gap-2">
          {!isDisabled && table.status === 'available' && (
            <button
              onClick={() => handlers.onSetStatus('cleaning')}
              className="px-3 py-3 text-[14px] text-white bg-blue-50 dark:bg-blue-900/20 rounded hover:bg-blue-100 transition-colors"
            >
              🧹 {t('cfClean') || 'Clean'}
            </button>
          )}
          {!isDisabled && table.status === 'cleaning' && (
            <button
              onClick={() => handlers.onSetStatus('available')}
              className="px-3 py-3 text-[14px] text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 rounded hover:bg-emerald-100 transition-colors"
            >
              ✓ {t('cfDone') || 'Done'}
            </button>
          )}
          ...
          <button
            onClick={handlers.onOpenHistory}
            className="flex items-center gap-1 px-3 py-3 text-[14px] text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 transition-colors"
          >
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-3 rounded hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 hover:text-slate-600 transition-colors"
          >
            <Edit2 size={12} />
          </button>

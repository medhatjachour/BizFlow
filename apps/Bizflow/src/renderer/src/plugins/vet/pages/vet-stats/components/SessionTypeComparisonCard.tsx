...
export function SessionTypeComparisonCard({ visitTypes }: { visitTypes: VisitTypeStat[] }) {
  ...
  const sorted = [...visitTypes].sort((a, b) =>
    metric === 'revenue' ? Number(b.revenue || 0) - Number(a.revenue || 0) : Number(b.count || 0) - Number(a.count || 0)
  )

  const maxVal = Math.max(1, ...sorted.map((x) => (metric === 'revenue' ? Number(x.revenue || 0) : Number(x.count || 0))))
    ...
    <div className="bg-white dark:bg-slate-800/80 rounded-2xl border border-slate-200/80 dark:border-slate-700/70 p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200">
          {t('vetSessionTypeComparison') || 'Session Breakdown'}
        </h3>
      ...
      <div className="space-y-3.5">
        {sorted.map((v) => {
          ...
          const share =
            metric === 'revenue'
              ? totalRevenue > 0 ? Math.round((Number(v.revenue || 0) / totalRevenue) * 100) : 0
              : totalCount > 0 ? Math.round((Number(v.count || 0) / totalCount) * 100) : 0

          return (
            <div key={v.visitType} className="space-y-1">
              <div className="flex items-center justify-between text-xs">
                <span className="font-semibold text-slate-700 dark:text-slate-300">{visitTypeLabel(v.visitType)}</span>
              ...
              <div className="h-2 bg-slate-100 dark:bg-slate-700/60 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-500 ${
                    hexColor(v.visitType) ? '' : VISIT_TYPE_BAR[v.visitType] ?? 'bg-teal-500'
                  }`}
                  style={{ width: `${pct}%`, backgroundColor: hexColor(v.visitType) || undefined }}
                />
              </div>
              <div className="flex items-center gap-2 text-[10px] text-slate-400 font-medium">
                ...
                <span>•</span>
                <span>avg {formatCurrency(v.avg)}</span>
              </div>
            </div>
          )
        })}
        ...
        {visitTypes.length === 0 && (
          <p className="text-xs text-slate-400 text-center py-6">{t('vetNoDataYet') || 'No session data yet'}</p>
        )}
      </div>
    </div>
  )

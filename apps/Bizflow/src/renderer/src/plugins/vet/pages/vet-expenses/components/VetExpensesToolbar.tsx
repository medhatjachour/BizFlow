import {
  ...
  Calendar, X,  ArrowUpDown
} from 'lucide-react'
import { PeriodPreset, ExpenseSortField, ExpenseViewMode } from '../types'
import { EXPENSE_CATEGORIES, PAYMENT_METHODS } from '../constants'
import { ExpenseHelpTooltip } from './ExpenseHelpTooltip'
import DateField from '@renderer/components/DateField'
import { useLanguage } from '@renderer/contexts/LanguageContext'

interface VetExpensesToolbarProps {
  period: PeriodPreset
  ...
  onSortFieldChange: (v: ExpenseSortField) => void
  sortAsc: boolean
  onToggleSortOrder: () => void
  viewMode: ExpenseViewMode
  onViewModeChange: (m: ExpenseViewMode) => void
  onAddExpense: () => void
  onRefresh: () => void
  isRefreshing: boolean
}

export function VetExpensesToolbar({
  period,
  onPeriodChange,
  customFrom = '',
  customTo = '',
  ...
  return (
    <div className="bg-white dark:bg-slate-800/90 backdrop-blur-md p-3.5 rounded-3xl border border-slate-200/80 dark:border-slate-700/80 shadow-sm space-y-3">
      {/* Row 1: Period Presets & Add Button */}
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Plus size={15} />
            <span>{isAr ? 'تسجيل مصروف جديد' : 'Add Expense'}</span>
          </button>
        </div>
      </div>

      {/* Row 2: Search, Filters & View Switcher */}
        ...
        <div className="flex flex-wrap items-center gap-2 flex-1 min-w-[280px]">
          ...
          <div className="relative flex-1 min-w-[200px] max-w-sm">
            ...
            <input
              ...
              placeholder={isAr ? 'بحث بالبيان، المورد، الملاحظات...' : 'Search by description, vendor, notes...'}
          ...
          <select
            ...
            className="px-2.5 py-1.5 text-xs rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-300 font-semibold focus:outline-none focus:ring-2 focus:ring-violet-500/20"
          >
            <option value="all">{isAr ? 'جميع طرق الدفع' : 'All Payment Methods'}</option>
          ...
          <div className="flex items-center gap-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-0.5 text-xs">
            <select
              value={sortField}
              onChange={(e) => onSortFieldChange(e.target.value as ExpenseSortField)}
            ...
            >
              ...
              <option value="amount">{isAr ? 'المبلغ' : 'Amount'}</option>

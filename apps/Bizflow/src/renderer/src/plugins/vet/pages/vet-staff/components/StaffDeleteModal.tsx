...
export function StaffDeleteModal({ staff, isDeleting, onConfirm, onClose }: StaffDeleteModalProps) {
    ...
    >
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-slate-900 dark:text-white text-sm">
                {isAr ? 'إزالة الطبيب البيطري' : 'Remove Veterinarian'}
              </h3>
              <p className="text-xs text-slate-400">{isAr ? 'تأكيد الحذف من الفريق' : 'Team registry removal'}</p>
            </div>
          </div>
        ...
        <p className="text-xs text-slate-600 dark:text-slate-300 mb-6 leading-relaxed">
          {isAr ? (
            <>
              هل أنت متأكد من رغبتك في إزالة الطبيب <strong>{staff.name}</strong> من قائمة الأطباء البيطريين؟ ستبقى السجلات الطبية والزيارات السابقة كما هي في النظام.
            </>
          ) : (
            <>
              Are you sure you want to remove <strong>{staff.name}</strong> from the clinical vet directory? All prior sessions and medical histories will remain intact.
            </>
          )}
        </p>
        ...
        <div className="flex gap-2.5">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 px-4 py-2.5 text-xs font-bold text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
          >
            {isAr ? 'إلغاء' : 'Cancel'}
          </button>
          <button
          ...
          >
            ...
            <span>{isDeleting ? (isAr ? 'جاري الإزالة...' : 'Removing…') : (isAr ? 'تأكيد الإزالة' : 'Remove')}</span>
          </button>
        </div>
      </div>
    </div>

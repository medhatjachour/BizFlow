...
interface Props {
  appointment: VetAppointmentRecord | null
  isDeleting: boolean
  onConfirm: () => void
  onClose: () => void
}

export function AppointmentDeleteModal({ appointment, isDeleting, onConfirm, onClose }: Props) {
    ...
    >
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-slate-900 dark:text-white text-sm">
                {isAr ? 'حذف الموعد' : 'Delete Appointment'}
              </h3>
              <p className="text-xs text-slate-400">{isAr ? 'إلغاء وإزالة الحجز' : 'Calendar booking removal'}</p>
            </div>
          </div>
        ...
        <p className="text-xs text-slate-600 dark:text-slate-300 mb-6 leading-relaxed">
          {isAr ? (
            <>
              هل أنت متأكد من حذف موعد <strong>{appointment.patient?.name || 'Patient'}</strong> في تمام الساعة{' '}
              <strong className="text-rose-600">{formatApptTime(appointment.appointmentDate)}</strong>؟
            </>
          ) : (
            <>
              Are you sure you want to delete the appointment for <strong>{appointment.patient?.name || 'Patient'}</strong> at{' '}
              <strong className="text-rose-600">{formatApptTime(appointment.appointmentDate)}</strong>?
            </>
          )}
        </p>

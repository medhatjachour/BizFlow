...
export function SessionDeleteModal({ session, isDeleting, onConfirm, onClose }: Props) {
    ...
    >
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-slate-900 dark:text-white text-sm">
                {isAr ? 'حذف الجلسة العلاجية' : 'Delete Clinical Session'}
              </h3>
              <p className="text-xs text-slate-400">{isAr ? 'إزالة السجل السريري' : 'Medical visit record'}</p>
            </div>
          </div>
        ...
        <p className="text-xs text-slate-600 dark:text-slate-300 mb-6 leading-relaxed">
          {isAr ? (
            <>
              هل أنت متأكد من حذف جلسة <strong>{session.patient?.name || 'General Visit'}</strong>؟ سيتم حذف بيانات التشخيص والوصفات الطبية الخاصة بها نهائياً.
            </>
          ) : (
            <>
              Are you sure you want to permanently remove the session record for <strong>{session.patient?.name || 'General Visit'}</strong>? All medical notes and prescriptions will be deleted.
            </>
          )}
        </p>

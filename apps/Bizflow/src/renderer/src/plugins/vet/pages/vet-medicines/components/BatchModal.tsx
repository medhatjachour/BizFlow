...
export const BatchModal: React.FC<BatchModalProps> = ({
  ...
  const hasSub = Boolean(subUnit && subUnitsPerContainer && subUnitsPerContainer > 0)
  ...
  async function applyAdjust() {
    ...
    if (!Number.isFinite(amount) || amount < 0 || (adjMode !== 'set' && amount <= 0)) {
      toast.error(t('vetAdjEnterAmount') || 'Enter a valid amount')
      return
    }
    setAdjBusy(true)
    try {
      await (window as any).api?.vet?.medicines?.adjustBatchStock(initial.id, {
        mode: adjMode,
        amount,
        unit: adjUnit === 'sub' ? 'sub' : 'container',
  ...
  function handleCostChange(v: string) {
    setForm(f => {
      ...
      if (!isNaN(pct) && newCost > 0) {
        return { ...f, costPerUnit: v, sellingPrice: (newCost * (1 + pct / 100)).toFixed(2) }
      }
  ...
  function handleMarginChange(v: string) {
    ...
    if (!isNaN(pct) && cost > 0) {
      setForm(f => ({ ...f, sellingPrice: (cost * (1 + pct / 100)).toFixed(2) }))
    }
  }
  ...
  const previewMargin =
    previewCost > 0 && previewSell > 0 ? ((previewSell - previewCost) / previewCost) * 100 : null

  async function submit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.expiryDate) {
      toast.error(t('vetExpiryDateRequired') || 'Expiry date is required')
      return
    }
    setBusy(true)
    try {
      ...
      if (initial) {
        await (window as any).api?.vet?.medicines?.updateBatch(initial.id, data)
      } else {
        await (window as any).api?.vet?.medicines?.addBatch(data)
      }
      toast.success(
        initial
          ? t('vetBatchUpdated') || 'Batch updated'
          : t('vetBatchReceived') || 'Batch received'
      )
      onSave()
    } catch (err: any) {
      ...
      <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-2xl overflow-y-auto max-h-[95vh]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
          <h2 className="font-bold text-slate-900 dark:text-white">
            {initial ? t('vetEditBatch') || 'Edit Batch' : t('vetReceiveBatch') || 'Receive New Batch'}
          </h2>
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">
                {t('vetBatchLotNum') || 'Batch / Lot #'}
              </label>
              <input
                value={form.batchNumber}
                onChange={e => setForm(f => ({ ...f, batchNumber: e.target.value }))}
                placeholder={t('vetOptional') || 'Optional'}
                className={INPUT_BASE_CLS}
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">
                {t('vetBatchSupplier') || 'Supplier'}
              </label>
              <input
                value={form.supplier}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">
                {t('vetBatchExpiryDate') || 'Expiry Date'} *
              </label>
              <DateField
                required
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">
                {t('vetBatchReceivedDate') || 'Received Date'}
              </label>
              <DateField
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">
                {t('vetBatchQtyLabel') || 'Qty'} ({unit}) {initial ? '' : '*'}
              </label>
              {initial ? (
                <div className={`${INPUT_BASE_CLS} bg-slate-50 dark:bg-slate-800/60 text-slate-500 cursor-not-allowed flex items-center`}>
          ...
          <div className="rounded-xl border border-violet-200 dark:border-violet-800/60 bg-violet-50/40 dark:bg-violet-950/20 p-4 space-y-3">
            ...
            {previewMargin !== null && (
              ...
              >
                Cost ${previewCost.toFixed(2)} → Sell ${previewSell.toFixed(2)} ={' '}
                {previewMargin >= 0 ? '+' : ''}
          ...
          {initial && (
            <div className="rounded-xl border border-amber-200 dark:border-amber-800/60 bg-amber-50/40 dark:bg-amber-950/20 p-4 space-y-3">
              ...
              <div className="flex items-center gap-1 p-1 bg-white dark:bg-slate-800 rounded-lg w-fit border border-slate-200 dark:border-slate-700">
                {(
                  [
                    ['add', t('vetAdjAdd') || 'Add', Plus],
                    ['remove', t('vetAdjRemove') || 'Remove', Minus],
                    ['set', t('vetAdjSet') || 'Set to', Settings2]
                  ] as const
                ).map(([m, label, Icon]) => (
                  <button
                    key={m}
              ...
              <div className="flex items-end gap-2">
                ...
                {hasSub && (
                  <div className="flex items-center gap-1 p-1 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 shrink-0">
                    <button
                      type="button"
                      onClick={() => setAdjUnit('container')}
                      className={`px-2 py-1 text-xs font-semibold rounded ${
                        adjUnit === 'container' ? 'bg-amber-600 text-white' : 'text-slate-500 dark:text-slate-400'
                      }`}
                    >
                      {unit}
                    </button>
                    <button
                      type="button"
                      onClick={() => setAdjUnit('sub')}
                      className={`px-2 py-1 text-xs font-semibold rounded ${
                        adjUnit === 'sub' ? 'bg-amber-600 text-white' : 'text-slate-500 dark:text-slate-400'
                      }`}
                    >
                      {subUnit}
                    </button>
              ...
              <input
                ...
                placeholder={t('vetAdjReasonPlaceholder') || 'Reason (optional)'}
                className={INPUT_BASE_CLS}
              />
              <button
                type="button"
                onClick={applyAdjust}
                disabled={adjBusy}
                className="w-full px-4 py-2 text-sm font-bold text-white bg-amber-600 hover:bg-amber-700 rounded-lg disabled:opacity-50 flex items-center justify-center gap-1.5"
              >
          ...
          <div className="flex gap-3 pt-1">
            ...
            >
              {busy ? (
                ...
                t('vetMedUpdate') || 'Update'
              ) : (
                t('vetAddBatch') || 'Add Batch'
              )}
            </button>
          </div>
        </form>

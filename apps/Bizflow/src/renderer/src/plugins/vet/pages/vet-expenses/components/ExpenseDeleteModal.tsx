...
export function ExpenseDeleteModal({ expense, isDeleting, onConfirm, onClose }: ExpenseDeleteModalProps) {
    ...
    >
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-slate-900 dark:text-white text-sm">
                {isAr ? 'حذف المصروف' : 'Delete Expense'}
              </h3>
              <p className="text-xs text-slate-400">{isAr ? 'تأكيد الحذف النهائي' : 'Permanent deletion'}</p>
            </div>
          </div>
        ...
        <p className="text-xs text-slate-600 dark:text-slate-300 mb-6 leading-relaxed">
          {isAr ? (
            <>
              هل أنت متأكد من حذف المصروف <strong>"{expense.description}"</strong> بمبلغ{' '}
              <strong className="text-rose-600">{formatExpenseMoney(expense.amount)}</strong>؟
            </>
          ) : (
            <>
              Are you sure you want to permanently delete <strong>"{expense.description}"</strong> (
              <strong className="text-rose-600">{formatExpenseMoney(expense.amount)}</strong>)?
            </>
          )}
        </p>

...
export function DeletePlanModal({ target, deleting, onClose, onConfirm }: DeletePlanModalProps) {
  const { t } = useLanguage()
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-sm p-6 space-y-4 text-center">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Existing active subscriptions using this package will remain unaffected.
          </p>
        </div>

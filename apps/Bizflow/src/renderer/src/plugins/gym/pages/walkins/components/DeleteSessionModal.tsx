...
export function DeleteSessionModal({
  ...
  const dt = formatSessionDateTime(target.date)
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-sm p-6 space-y-4 text-center">
        ...
        <div>
          <h3 className="font-bold text-base text-slate-900 dark:text-white">
            {t('gymDeleteSession') || 'Remove Session Record'}
          </h3>
          <p className="text-xs text-slate-400 mt-1">
            {target.trainee?.name || 'Guest Visit'} · {dt.date} {dt.time}
          </p>
        </div>

        <p className="text-xs text-slate-600 dark:text-slate-300">
          Are you sure you want to remove this visit record? Any associated cash flow calculations will be affected.
        </p>
        ...
        <div className="flex gap-2.5 pt-2">
          ...
          >
            ...
            <span>{t('gymDelete') || 'Delete Record'}</span>
          </button>
        </div>
      </div>
    </div>

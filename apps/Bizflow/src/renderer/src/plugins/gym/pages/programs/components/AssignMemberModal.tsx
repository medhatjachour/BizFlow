...
export function AssignMemberModal({ program, isOpen, onClose, onSaved }: AssignMemberModalProps) {
  const { t } = useLanguage()
  const {
    form,
    setForm,
    ...
    clearMember,
    handleSubmit
  } = useAssignProgram(program, isOpen, onSaved, onClose)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-md overflow-hidden">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">
                {t('gymAssignMemberToProgram') || 'Assign Routine to Member'}
              </h3>
              <p className="text-xs text-slate-400 truncate max-w-[240px]">{program.name}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder={t('gymProgramSearchMember') || 'Search active member by name or phone...'}
          ...
          <div>
            <label className={labelCls}>{t('gymNotes') || 'Custom Member Instructions'}</label>
            <textarea
              className={inputCls}
              rows={2}
              value={form.notes}
              onChange={e => setForm(prev => ({ ...prev, notes: e.target.value }))}
              placeholder="e.g. Focus on hamstring warmups, substitute front squats..."
            />
          </div>
          ...
          <div className="flex gap-2.5 pt-2">
            ...
            >
              ...
              <span>{t('gymAssign') || 'Assign Routine'}</span>
            </button>
          </div>
        </form>
      </div>

...
import { TraineeFilter, ViewMode } from '../types'
import { FILTER_OPTIONS } from '../constants'
import { useLanguage } from '@renderer/contexts/LanguageContext'

interface TraineeToolbarProps {
  searchInput: string
  onSearchInputChange: (val: string) => void
  onSearchSubmit: (e: React.FormEvent) => void
  onClearSearch: () => void
  onAddNew: () => void
  activeFilter: TraineeFilter
  onFilterChange: (f: TraineeFilter) => void
  viewMode: ViewMode
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <form onSubmit={onSearchSubmit} className="flex-1 flex items-center gap-2 min-w-[280px]">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder={t('gymSearchTrainees') || 'Search by name, phone, or email...'}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Plus size={15} />
            <span>{t('gymAddTrainee') || 'Add Member'}</span>
          </button>
        </div>
      </div>

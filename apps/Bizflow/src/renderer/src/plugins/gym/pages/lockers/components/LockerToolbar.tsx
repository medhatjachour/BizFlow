...
import { LockerZoneFilter, LockerViewMode } from '../types'
...
interface LockerToolbarProps {
  ...
  activeZone: LockerZoneFilter
  onZoneChange: (z: LockerZoneFilter) => void
  viewMode: LockerViewMode
  onViewModeChange: (m: LockerViewMode) => void
  onAddNew: () => void
  totalCount: number
}
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[280px]">
          ...
          <input
            ...
            placeholder="Search by locker number, member name, or phone..."
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Plus size={15} />
            <span>{t('gymAddLocker') || 'Add Locker'}</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-2 overflow-x-auto pb-1">
        <div className="flex items-center gap-1.5">
          {ZONE_FILTERS.map(zf => {
            const isSelected = activeZone === zf.id
            return (
              <button
                key={zf.id}
                onClick={() => onZoneChange(zf.id)}
        ...
        <span className="text-xs font-semibold text-slate-400 whitespace-nowrap pl-2">
          {totalCount} {totalCount === 1 ? 'locker' : 'lockers'} in view
        </span>
      </div>
    </div>
  )

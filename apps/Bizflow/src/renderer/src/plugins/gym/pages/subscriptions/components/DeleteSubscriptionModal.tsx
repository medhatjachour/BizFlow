...
export function DeleteSubscriptionModal({ target, onClose, onConfirm }: DeleteSubscriptionModalProps) {
  const { t } = useLanguage()
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-sm p-6 space-y-4 text-center">
        ...
        <div>
          <h3 className="font-bold text-base text-slate-900 dark:text-white">
            {t('gymDeleteSubscription') || 'Delete Subscription'}
          </h3>
          <p className="text-xs text-slate-400 mt-1">
            {target.trainee?.name} · {target.plan?.name}
          </p>
        </div>

        <p className="text-xs text-slate-600 dark:text-slate-300">
          Are you sure you want to remove this subscription contract? This action cannot be reverted.
        </p>
        ...
        <div className="flex gap-2.5 pt-2">
          ...
          <button
            type="button"
            onClick={onConfirm}
            className="flex-1 px-4 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold transition-all shadow-sm"
          >
            {t('gymDelete') || 'Delete'}
          </button>
        </div>

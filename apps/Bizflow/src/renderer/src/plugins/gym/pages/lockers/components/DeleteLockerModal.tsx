      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-sm p-6 space-y-4 text-center">
        ...
        <div>
          <h3 className="font-bold text-base text-slate-900 dark:text-white">
            Delete Locker {target.number}?
          </h3>
          <p className="text-xs text-slate-400 mt-1">
            Section: <span className="capitalize">{target.zone}</span>
          </p>
        </div>

        <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
          Are you sure you want to permanently remove this locker unit from your facility registry?
        </p>
        ...
        <div className="flex gap-2.5 pt-2">
          ...
          >
            ...
            <span>{t('gymDelete') || 'Delete Locker'}</span>
          </button>
        </div>
      </div>
    </div>

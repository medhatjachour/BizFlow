...
export const DeleteRecipeModal: React.FC<Props> = ({ isOpen, onClose, onConfirm }) => {
  const { t } = useLanguage()
    ...
    >
      ...
      >
        ...
        <h3 className="text-base font-bold text-slate-900 dark:text-white">
          {t('bakeryArchiveRecipeTitle') || 'Archive Recipe'}
        </h3>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          {t('bakeryArchiveConfirm') ||
            'Are you sure you want to archive this recipe? Historical production batches and sales will remain intact.'}
        </p>
        ...
        <div className="flex items-center justify-end gap-3 mt-6">
          ...
          <button
            onClick={onConfirm}
            className="px-4 py-2 text-sm font-bold bg-rose-600 hover:bg-rose-700 text-white rounded-xl shadow-sm"
          >
            {t('bakeryArchiveBtn') || 'Archive'}
          </button>
        </div>
      </div>
    </div>

...
export const DeleteSaleConfirmModal: React.FC<Props> = ({ isOpen, onClose, onConfirm }) => {
  const { t } = useLanguage()
    ...
    >
      ...
      >
        ...
        <h3 className="text-base font-bold text-slate-900 dark:text-white">
          {t('bakerySaleDeleteTitle') || 'Delete Sale Record'}
        </h3>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          {t('bakerySaleDeleteConfirm') ||
            'Are you sure you want to delete this sale? Note: Associated batch sold quantities will be reversed.'}
        </p>
        ...
        <div className="flex items-center justify-end gap-3 mt-6">
          ...
          <button
            onClick={onConfirm}
            className="px-4 py-2 text-sm font-bold bg-rose-600 hover:bg-rose-700 text-white rounded-xl transition-colors shadow-sm"
          >

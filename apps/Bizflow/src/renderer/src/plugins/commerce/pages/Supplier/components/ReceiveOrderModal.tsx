    ...
    <div className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs">
      <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl p-5 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="text-sm font-bold text-slate-900 dark:text-white">{t('receiveOrder') || 'Receive & Reconcile PO'}</h4>
            <p className="text-xs text-slate-500">{t('poIdentifier') || 'PO Number'}: {order.poNumber}</p>
          </div>
        </div>
        ...
        <div className="p-3 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/50 rounded-xl flex items-start gap-2.5 text-xs text-amber-800 dark:text-amber-300">
          ...
          <span>
            {t('reconcileAndAcceptInventory') || 'Executing this action will immediately increment physical inventory stock counts for all item variants listed under this purchase order.'}
          </span>
        </div>

        <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl space-y-1.5 text-xs">
          <div className="flex justify-between">
            <span className="text-slate-500">{t('supplierPartner') || 'Supplier:'}</span>
            <span className="font-semibold text-slate-900 dark:text-white">{order.supplier?.name}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-500">{t('totalAmount') || 'Total Valuation:'}</span>
            <span className="font-bold text-emerald-600 dark:text-emerald-400 font-mono">
              {formatCurrency(order.totalAmount)}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-500">{t('quantity') || 'Items Quantity:'}</span>

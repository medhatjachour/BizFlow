...
export function registerOrderHandlers(prisma: any) {
    ...
    try {
      return await prisma.dineInOrder.findUnique({
        ...
        include: {
          table: true,
          items: {
            include: { menuItem: { include: { modifierGroups: { include: { options: true } } } } },
            orderBy: { createdAt: 'asc' }
          },
          payments: true
        }
  ...
  ipcMain.handle('restaurant:openOrder', async (_e, data: {
    ...
    try {
      ...
      if (existingOpen) {
        throw new Error('This table already has an active order — settle or void it before opening a new one')
      }
  ...
  ipcMain.handle('restaurant:addOrderItem', async (_e, data: {
    ...
    try {
      ...
      const totalPrice = data.totalPrice !== undefined ? Number(data.totalPrice) : unitPrice * quantity
  ...
  ipcMain.handle('restaurant:updateOrderItem', async (_e, data: {
    ...
    try {
      ...
      const qty = rest.quantity !== undefined ? Number(rest.quantity) : current.quantity
      const total = rest.totalPrice !== undefined ? Number(rest.totalPrice) : current.unitPrice * qty
  ...
  ipcMain.handle('restaurant:processPayment', async (_e, data: {
    ...
    try {
      ...
      const allPayments = await prisma.orderPayment.findMany({ where: { orderId: data.orderId } })
      const paidTotal = allPayments.reduce((s: number, p: any) => s + p.amount, 0)

      const order = await prisma.dineInOrder.findUnique({ where: { id: data.orderId } })
      ...
      // If fully paid, settle and release table
      if (paidTotal >= order.total) {
        await prisma.dineInOrder.update({
          where: { id: data.orderId },
          data: { status: 'paid', paymentMethod: data.paymentMethod, closedAt: new Date() }
        })
        await prisma.restaurantTable.update({
          where: { id: order.tableId },
          data: { status: 'cleaning' } // Marked cleaning for rapid turnover
        })
      } else {
        await prisma.dineInOrder.update({
          where: { id: data.orderId },
      ...
      return { payment, paidTotal, remaining: Math.max(0, order.total - paidTotal) }
    } catch (err) {
